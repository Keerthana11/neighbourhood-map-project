...
var ViewModel = function() {
    ...
    self.locationsList().forEach(function(filteritem){
    	//for ( x = 0; x < self.locationsList().length; x++) {

    		if(self.locationsList().nameTitle.toLowerCase().indexOf(value.toLowerCase()) >= 0) {
        		self.filteredList().push(self.locationsList());
  ...
  /*this.filteredList = ko.computed(function(){
  	...
  	if (!filter) {
  		...
  		return ko.utils.arrayFilter(self.locationsList(), function(item){
  			return item.nameTitle().toLowerCase().indexOf(filter) != -1;
  		});
  	}
  });*/
